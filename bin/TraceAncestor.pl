...
GetOptions (
	...
	'y|hybrid=s'    => \$hybrid, ## particular hybrid you want to focus on (optional). If -f not indicated.
    'o|outdir'      => \$dirout,
	'h|help!'       => \$help
)
...
if (!$hybrid){
    warn $usage;
	warn "\nWarn :: --hybrid is empty.  particular hybrid you want to focus on\n\n";
    exit 0; 
} 
#------------------------------------------------------------------------------------------------------------------------------------------------------
...
my $file_curve = $dirout."/". $hybrid."_curve.txt";
my $file_ideogram = $dirout."/". $hybrid."_ideo.txt";
my $file_color = $dirout."/". $hybrid."_color.txt";
my $file_chromosome = $dirout."/". $hybrid."_chrom.txt";
my $file_ancestor = $dirout."/". $hybrid."_ancestor.txt";
...
for my $parent (sort keys(%blocspos)){
	for my $chro (sort keys(%{$blocspos{$parent}})){
		for my $num (sort {$a<=>$b} keys(%{$blocspos{$parent}{$chro}})){
			...
			my $end = $position[1];
			print Ffreq join("\t",$hybrid , $parent,$chro ,$start_cor,$end ,$percentage{$parent}{$hybrid}{$chro}{$num}),"\n"; 
            push @{$data{$chro}} , {
                start => $start_cor,
                end   => $end,
                hybrid => $parent,
                percent => $percentage{$parent}{$hybrid}{$chro}{$num} 
            }; 
...
open(GEMO, ">$file_curve") or die("cant write $file_curve\n"); # output => frequency of ancestral alleles along chromosomes
print GEMO join("\t","chr","start","end",join("\t",@list_parent)),"\n"; # header
...
foreach my $chromosome (sort keys %coordinate) {
    ...
    foreach my $start (sort {$a <=> $b}  @{$coordinate{$chromosome}}) {
        if ($last) {
            ...
            print GEMO join("\t",$id, $last,$end,join("\t",@value)) ,"\n";
            $last = $start;
        }
        else {
            $last = $start;          
	...
	for my$p (sort {$a<=>$b} keys %{$hasher{$CHROMOSOMES}}){
		...
		for my$slice (sort {$a<=>$b} keys %{$hasher{$CHROMOSOMES}{$p}}){
			...
			if ($color ne $hasher{$CHROMOSOMES}{$p}{$slice}){
				...
				if ($endS <= $endC){
					if($color ne "NA") {
						print F11 join("\t",$chro_renamed ,$p ,$begin ,$endS ,$color),"\n";
					}
				}
				else{
					if($color ne "NA"){
						print F11 join("\t",$chro_renamed, $p ,$begin ,$endC ,$color),"\n";
					}
				}
				$begin = $slice + 1;
			}
...
for my$chro(sort keys %endChro){
	my $chro_renamed; 
	if ($chro < 10) {
		$chro_renamed = "0" .$chro;
	}
	else {$chro_renamed = $chro}

	if ($ploidy == 2){
		print CHROMOSOME "$chro_renamed\t$endChro{$chro}\t\t\t01\n";
	}
	if ($ploidy == 3){
		print CHROMOSOME "$chro_renamed\t$endChro{$chro}\t\t\t012\n";
	}
	if ($ploidy == 4){
		print CHROMOSOME "$chro_renamed\t$endChro{$chro}\t\t\t0123\n";
	}
}
close CHROMOSOME; 
